Write-Host "Starting ClustEngine installation..." -ForegroundColor Cyan

# Navigate to the project root directory
$projectRoot = Resolve-Path "$PSScriptRoot\..\..\.."
Set-Location -Path $projectRoot

Write-Host "Working directory set to project root: $((Get-Location).Path)"
...
if (-Not (Test-Path "gradlew.bat")) {
    Write-Host "Error: 'gradlew.bat' wrapper not found in the current directory." -ForegroundColor Red
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Error: Gradle build failed. Check the logs above for details." -ForegroundColor Red
    exit 1
}
...
foreach ($file in $files) {
    if (-Not (Test-Path -Path $file)) {
        ...
        Write-Host "Suggestion: Please ensure the build was successful and all files exist."
        exit 1
    }
}
...
@echo off
cd /d "$installDir"
java -Dkotlinx.coroutines.io.parallelism=512 -jar "$installDir\GuiAPP-1.0.0.jar"
"@
