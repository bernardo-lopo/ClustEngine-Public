...
echo "Starting ClustEngine installation..."

# Navigate to the project root directory
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" &> /dev/null && pwd)"
cd "$SCRIPT_DIR/../../.." || { echo -e "${RED}Error: Could not navigate to project root.${NC}"; exit 1; }

echo "Working directory set to project root: $(pwd)"
...
# Build of the project using Gradle wrapper
echo "Building the project with Gradle..."
if [ ! -f "gradlew" ]; then
    echo -e "${RED}Error: 'gradlew' wrapper not found in the current directory.${NC}"
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo -e "${RED}Error: Gradle build failed. Check the logs above for details.${NC}"
    exit 1
fi
...
for file in "${FILES[@]}"; do
    if [ ! -e "$file" ]; then
        ...
        echo "Suggestion: Please ensure the build was successful and all files exist."
        exit 1
    fi
done
...
#!/bin/bash
cd "$INSTALL_DIR"
java -Dkotlinx.coroutines.io.parallelism=512 -jar "$INSTALL_DIR/GuiAPP-1.0.0.jar"
EOF
